        ...
        {seasons.map((season) => (
          <article key={season.id} className="rounded-2xl border border-gray-800 bg-gray-900 p-5 shadow-xl">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {season.status === "DRAFT" && <form action={seasonAction}><input type="hidden" name="seasonId" value={season.id} /><button name="action" value="archive" className="rounded-xl border border-red-400/50 px-3 py-2 font-semibold text-red-200 hover:bg-red-400/10">Eliminar borrador</button></form>}
              ...
              {season.status === "CLOSED" && <form action={seasonAction}><input type="hidden" name="seasonId" value={season.id} /><button name="action" value="archive" className="rounded-xl border border-red-400/50 px-3 py-2 font-semibold text-red-200 hover:bg-red-400/10">Archivar temporada</button></form>}
            </div>
