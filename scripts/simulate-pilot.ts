...
try {
  ...
  const lateUser = await prisma.user.upsert({ where: { emailNormalized: "tardio-piloto@example.invalid" }, update: { isTestUser: true, status: "APPROVED" }, create: { firstName: "Usuario", lastName: "Tardío", nickname: "tardio-piloto", nicknameNormalized: "tardio-piloto", email: "tardio-piloto@example.invalid", emailNormalized: "tardio-piloto@example.invalid", passwordHash: "pilot-not-for-login", role: "USER", status: "APPROVED", emailVerifiedAt: new Date(), approvedAt: new Date(), isTestUser: true, favoriteTeam: { connect: { id: (await prisma.team.findFirstOrThrow()).id } } } });
